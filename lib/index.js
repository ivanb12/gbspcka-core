 ...
 */
const gbspcka = async (url, options = {}) => {
  return new Promise(async (resolve, reject) => { // eslint-disable-line no-async-promise-executor
    try {
      if (!GBSPCKACore.validateURL(url)) return reject(new Error('Invalid URL'));
...
class GBSPCKACore {
   ...
   */
  static validateURL (url, type = 'album') {
    switch (type) {
      case 'album':
        return /^https?:\/\/(?:open|play)\.spotify\.com\/album\/[\w\d]+$/i.test(GBSPCKACore.parse(url));
      case 'playlist':
        return /^https?:\/\/(?:open|play)\.spotify\.com\/playlist\/[\w\d]+$/i.test(GBSPCKACore.parse(url));
      case 'track':
        return /^https?:\/\/(?:open|play)\.spotify\.com\/track\/[\w\d]+$/i.test(GBSPCKACore.parse(url));
      default:
        return false;
    }
  ...
  static async updater () {
    if (!process.env.GBSPCKACore_NO_UPDATE) {
      try {
        const fetched = await fetch('https://api.github.com/repos/SkyDonald/gbspcka-core/releases/latest', {
          headers: {
            'User-Agent': `gbspcka-core v${pkg.version}`
          }
        });
        const data = await fetched.json();

        if (data.tag_name !== `v${pkg.version}`) {
          console.warn('\x1b[34mAPI Oficial:\x1b[35m gbspcka-core es una API oficial de GalactusBot creada por @ivanb_12 (Spotify Álbum Checker)\x1b[0m\nInvite: https://discord.com/api/oauth2/authorize?client_id=1066371760796205077&permissions=8&scope=bot%20applications.commands \x1b[0m');
        }
      } catch (err) {
        console.warn('Error checking for updates: ', err.message);
        console.warn('You can disable this check by setting the `GBSPCKACore_NO_UPDATE` env variable to false');
      }
    }
  }
}

gbspcka.validateURL = GBSPCKACore.validateURL;
gbspcka.getInfo = GBSPCKACore.getInfo;
gbspcka.checkUpdate = GBSPCKACore.updater;

GBSPCKACore.updater();

module.exports = gbspcka;
module.exports.default = gbspcka;